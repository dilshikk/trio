    ...
    <section ref={ref} className="relative py-32 px-6 md:px-16 lg:px-24 border-t border-white/6">
      <div className="max-w-[1400px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9 }} className="mb-20">
          <p className="text-[10px] tracking-[0.35em] text-white/30 uppercase mb-4"><EditableText tKey="process.eyebrow" /></p>
          <h2 className="text-[clamp(32px,4.5vw,64px)] font-bold tracking-[-0.03em] text-white leading-[0.95]">
            <EditableText tKey="process.title" /><br />
            <span className="text-white/25"><EditableText tKey="process.titleFaded" /></span>
          </h2>
        </motion.div>
            ...
            {STEPS.map((num, i) => (
              <motion.div key={num} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.8, delay: i * 0.1, ease: [0.25, 0.1, 0.25, 1] as const }} className="group flex items-start gap-8 py-8 border-b border-white/6 hover:border-white/15 transition-colors duration-500">
                ...
                <div className="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-12">
                  <h3 className="text-[clamp(20px,2.5vw,36px)] font-bold tracking-[-0.02em] text-white w-48 flex-shrink-0">
                    <EditableText tKey={`process.${num}.label`} />
                  </h3>
                  <p className="text-[14px] text-white/40 leading-relaxed font-light max-w-md group-hover:text-white/60 transition-colors duration-500">
                    <EditableText tKey={`process.${num}.desc`} />
                  </p>
                </div>
              </motion.div>
            ))}

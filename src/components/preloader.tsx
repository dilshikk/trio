    ...
    <AnimatePresence>
      {!exitPhase ? (
        ...
        >
          ...
          />
          {[28, 50, 72].map((top, i) => (
            <motion.div key={i} className="absolute left-0 right-0 h-[1px] bg-white pointer-events-none" style={{ top: `${top}%` }}
              initial={{ scaleX: 0, opacity: 0 }} animate={{ scaleX: [0, 1, 0], opacity: [0, 0.07, 0] }}
          ...
          <motion.div
            ...
            animate={{ scale: [0.03, 0.10, 0.42, 0.36], opacity: [0, 0.6, 1, 1], filter: ["blur(24px)", "blur(10px)", "blur(0px)", "blur(0px)"] }}
            transition={{ duration: 2.6, times: [0, 0.22, 0.72, 1], ease: [0.16, 1, 0.3, 1] as const, delay: 0.15 }}
          >
            <img src="https://hercules-cdn.com/file_exMhy8nexpXEXJmG0mlYSQKH" alt="TRIO GROUPS"

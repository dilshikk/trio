...
function PremiumButton({ loading, children }: { loading: boolean; children: React.ReactNode }) {
  ...
  return (
    <motion.button ref={btnRef} type="submit" disabled={loading} onClick={addRipple} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} className="relative w-full overflow-hidden py-3.5 text-sm font-bold tracking-[0.15em] uppercase rounded-lg disabled:opacity-60 cursor-pointer" style={{ background: "linear-gradient(135deg, #6395ff 0%, #a78bfa 50%, #f472b6 100%)", boxShadow: "0 0 30px rgba(99,149,255,0.35), 0 0 60px rgba(167,139,250,0.15)" }}>
      <motion.span className="absolute inset-0" style={{ background: "linear-gradient(105deg, transparent 30%, rgba(255,255,255,0.25) 50%, transparent 70%)", backgroundSize: "200% 100%" }} animate={{ backgroundPosition: ["200% 0", "-200% 0"] }} transition={{ duration: 1.8, repeat: Infinity, ease: "linear" as const }} />
      {ripples.map((r) => (<motion.span key={r.id} className="absolute rounded-full bg-white/30 pointer-events-none" style={{ left: r.x, top: r.y, width: 8, height: 8, marginLeft: -4, marginTop: -4 }} initial={{ scale: 0, opacity: 1 }} animate={{ scale: 28, opacity: 0 }} transition={{ duration: 0.7, ease: "easeOut" as const }} />))}
      <span className="relative z-10 flex items-center justify-center gap-2 text-white">
        {loading ? (<><motion.span animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: "linear" as const }} className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full block" />Проверяем…</>) : (<><Zap className="w-4 h-4" />{children}</>)}
      </span>
    </motion.button>
  );
}
...
function FloatingParticle({ delay, x, size }: { delay: number; x: number; size: number }) {
  return (
    <motion.div className="absolute rounded-full pointer-events-none" style={{ left: `${x}%`, bottom: "-10px", width: size, height: size, background: "linear-gradient(135deg, #6395ff40, #a78bfa40)" }} animate={{ y: ["-0vh", "-100vh"], opacity: [0, 0.6, 0] }} transition={{ duration: 6 + delay, delay, repeat: Infinity, ease: "easeOut" as const }} />
  );
}
...
export default function AdminLogin({ onLogin }: Props) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => { mouseX.set((e.clientX / window.innerWidth - 0.5) * 30); mouseY.set((e.clientY / window.innerHeight - 0.5) * 30); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch (err) { setError(err instanceof Error ? err.message : "Ошибка входа."); setShake(true); setTimeout(() => setShake(false), 600); }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center overflow-hidden relative" style={{ background: "oklch(0.06 0 0)" }}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">{particles.map((x, i) => <FloatingParticle key={x} x={x} delay={i * 0.4} size={3 + (i % 4) * 2} />)}</div>
      ...
      <motion.div initial={{ opacity: 0, y: 40, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] as const }} className="relative z-10 w-full max-w-md mx-4">
            ...
            <motion.div className="flex flex-col items-center gap-3" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.7 }}>
              <div className="relative">
                ...
                <div className="relative w-16 h-16 rounded-full flex items-center justify-center" style={{ background: "linear-gradient(135deg, #1a1a2e, #16213e)" }}>
                  <img src="/images/emblem.png" alt="TRIO GROUPS" className="w-9 h-9 object-contain" />
                </div>
              </div>
            ...
            <form onSubmit={(e) => { void handleSubmit(e); }} className="space-y-4">
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.45, duration: 0.6 }} className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <button type="button" onClick={() => setShowPass((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/70 transition-colors cursor-pointer">{showPass ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
              ...
              <AnimatePresence>{error && (<motion.div initial={{ opacity: 0, y: -8, height: 0 }} animate={{ opacity: 1, y: 0, height: "auto" }} exit={{ opacity: 0, y: -8, height: 0 }} transition={{ duration: 0.3 }} className="flex items-center gap-2 text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2.5"><span className="w-1.5 h-1.5 rounded-full bg-red-400 shrink-0 animate-pulse" />{error}</motion.div>)}</AnimatePresence>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.6 }}><PremiumButton loading={loading}>Войти</PremiumButton></motion.div>
            </form>
          </div>
        </motion.div>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }} className="text-center text-[10px] text-white/20 mt-6 tracking-wider">
          MADE IN <a href="https://dscreative.uz" target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-[#6395ff] transition-colors duration-300 font-semibold tracking-[0.1em]">DS CREATIVE</a>
        </motion.p>
      </motion.div>
    </div>
  );

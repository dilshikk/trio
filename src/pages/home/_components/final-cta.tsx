        ...
        <AnimatePresence mode="wait">
          {!showForm ? (
            <motion.div key="cta" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="text-center">
              <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="text-[10px] tracking-[0.35em] text-white/30 uppercase mb-10">
                <EditableText tKey="cta.eyebrow" />
              </motion.p>
              <div className="overflow-hidden"><motion.h2 initial={{ y: 100, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1.1, ease: [0.16, 1, 0.3, 1] as const }} className="text-[clamp(44px,8vw,120px)] font-bold leading-[0.9] tracking-[-0.04em] text-white mb-3">
                <EditableText tKey="cta.title" />
              </motion.h2></div>
              <div className="overflow-hidden"><motion.h2 initial={{ y: 100, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1.1, delay: 0.08, ease: [0.16, 1, 0.3, 1] as const }} className="text-[clamp(44px,8vw,120px)] font-bold leading-[0.9] tracking-[-0.04em] text-white/25 mb-16">
                <EditableText tKey="cta.titleFaded" />
              </motion.h2></div>
              <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, delay: 0.3 }} className="text-[16px] text-white/45 font-light max-w-lg mx-auto mb-14 leading-relaxed">
                <EditableText tKey="cta.desc" />
              </motion.p>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.45 }} className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <button onClick={() => setShowForm(true)} className="group relative overflow-hidden border border-white/80 text-white text-[11px] font-semibold tracking-[0.25em] uppercase px-10 py-4 hover:border-white transition-colors duration-500" data-cursor="WRITE">
                  <span className="relative z-10 group-hover:text-black transition-colors duration-300"><EditableText tKey="cta.button" /></span>
                  <span className="absolute inset-0 bg-white scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500" />
                </button>
                <a href="#about" onClick={(e) => { e.preventDefault(); document.querySelector("#about")?.scrollIntoView({ behavior: "smooth" }); }} className="text-[11px] font-medium tracking-[0.2em] uppercase text-white/40 hover:text-white/70 transition-colors duration-300">
                  <EditableText tKey="cta.learnMore" /> ↗
                </a>
              </motion.div>
            </motion.div>
          ) : (
            <motion.div key="form" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] as const }}>
              <div className="flex items-center justify-between mb-12">
                <div>
                  <p className="text-[10px] tracking-[0.35em] text-white/30 uppercase mb-3"><EditableText tKey="cta.formEyebrow" /></p>
                  <h2 className="text-[clamp(28px,4vw,52px)] font-bold tracking-[-0.03em] text-white leading-[0.95]">
                    <EditableText tKey="cta.formTitle" /><br />
                    <span className="text-white/30"><EditableText tKey="cta.formTitleFaded" /></span>
                  </h2>
                </div>
                <button onClick={() => setShowForm(false)} className="text-[10px] tracking-[0.2em] uppercase text-white/30 hover:text-white/60 transition-colors duration-300 border border-white/10 hover:border-white/30 px-4 py-2">
                  <EditableText tKey="cta.back" />
                </button>
              </div>
              <ContactForm />
            </motion.div>

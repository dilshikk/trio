...
const services: ServiceOption[] = ["LOGISTICS", "ACCOUNTING", "CONSULTING", "ALL THREE"];
      ...
      <AnimatePresence mode="wait">
        {submitted ? (
          <motion.div key="success" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] as const }} className="text-center py-16 space-y-6">
            ...
            <p className="text-[10px] tracking-[0.35em] uppercase text-white/30">MESSAGE RECEIVED</p>
            <h3 className="text-[clamp(28px,4vw,52px)] font-bold tracking-[-0.03em] text-white leading-[0.95]">WE'LL BE IN<br /><span style={{ color: accentColor }}>TOUCH SOON.</span></h3>
            <p className="text-[14px] text-white/40 font-light max-w-sm mx-auto leading-relaxed">Thank you for reaching out. A member of our team will contact you within one business day.</p>
            <button onClick={() => { setSubmitted(false); setForm({ name: "", company: "", email: "", message: "", service: "" }); }} className="mt-6 text-[10px] tracking-[0.25em] uppercase text-white/30 hover:text-white/60 transition-colors duration-300 border-b border-white/10 hover:border-white/30 pb-0.5">SEND ANOTHER MESSAGE</button>
          </motion.div>
        ) : (
          <motion.form key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-3">
              <label className="text-[9px] tracking-[0.3em] uppercase text-white/30">Service of Interest</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Name" placeholder="John Smith" value={form.name} error={errors.name} onChange={(v) => { setForm((f) => ({ ...f, name: v })); setErrors((e) => ({ ...e, name: "" })); }} accentColor={accentColor} />
              <Field label="Company" placeholder="Your Company" value={form.company} onChange={(v) => setForm((f) => ({ ...f, company: v }))} accentColor={accentColor} />
            </div>
            <Field label="Email" placeholder="example@company.com" type="email" value={form.email} error={errors.email} onChange={(v) => { setForm((f) => ({ ...f, email: v })); setErrors((e) => ({ ...e, email: "" })); }} accentColor={accentColor} />
            <div className="space-y-2">
              <label className="text-[9px] tracking-[0.3em] uppercase text-white/30">Message</label>
              <textarea rows={4} placeholder="Tell us about your project or challenge..." value={form.message} onChange={(e) => { setForm((f) => ({ ...f, message: e.target.value })); setErrors((er) => ({ ...er, message: "" })); }} className="w-full bg-transparent border text-white text-[13px] font-light px-4 py-3 outline-none resize-none transition-all duration-300 placeholder:text-white/20" style={{ borderColor: errors.message ? "rgba(248,113,113,0.5)" : "rgba(255,255,255,0.12)" }} onFocus={(e) => { e.currentTarget.style.borderColor = accentColor + "60"; }} onBlur={(e) => { e.currentTarget.style.borderColor = errors.message ? "rgba(248,113,113,0.5)" : "rgba(255,255,255,0.12)"; }} />
            ...
            <motion.button type="submit" disabled={submitting} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="group relative w-full overflow-hidden border text-white text-[11px] font-semibold tracking-[0.25em] uppercase px-10 py-4 transition-colors duration-500 disabled:opacity-60" style={{ borderColor: accentColor + "80" }}>
              <span className="relative z-10 group-hover:text-black transition-colors duration-300">{submitting ? "SENDING..." : "SEND MESSAGE"}</span>
